...
begin
open ref1 for
    SELECT usr_id, usr_usern, usr_lastn, usr_firstn, usr_email, usr_privileges, server_gen_pwd FROM users WHERE usr_id > 0 and (get_usr_id IS NULL or usr_id = get_usr_id) AND usr_is_deleted = false ORDER BY usr_firstn, usr_lastn, usr_usern;
return next ref1;
end;
$$ language plpgsql;
